...
const SkillIcon = ({ icon: Icon, name, level, isSoft }) => (
  ...
  >
    <motion.div
      className={`w-20 h-20 ${
        isSoft ? "bg-green-100" : "bg-blue-100"
      } rounded-full flex items-center justify-center mb-2`}
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.9 }}
    >
      <Icon
        className={`text-4xl ${isSoft ? "text-green-600" : "text-blue-600"}`}
      />
    </motion.div>
        ...
        {index < skills.length - 1 && (
          <motion.div
            className={`hidden md:block w-8 h-0.5 ${
              isSoft ? "bg-green-300" : "bg-blue-300"
            }`}
            initial={{ width: 0 }}
            animate={{ width: "2rem" }}
...
const Skills = () => {
  const technicalSkills = [
    ...
    { name: "TypeScript", icon: BiLogoTypescript, level: 4 },
    { name: "CSS", icon: FaCss3, level: 4 },
    { name: "HTML", icon: FaHtml5, level: 4 },
    { name: "RTL Jest", icon: SiJest, level: 3 },
